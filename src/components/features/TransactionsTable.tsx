		...
		() => [
			{
				...
				columns: [
					...
					{
						Header: 'Status',
						accessor: ({status}) => status ? <span style={{color: 'green'}}>Success</span> : <span style={{color: 'red'}}>Failed</span>,
					},
					{
						Header: 'Pre balance',
					...
					},
					{
						...
						accessor: ({balanceChange}) => !balanceChange ? balanceChange : <span style={{color: balanceChange >= 0 ? 'green' : 'red'}}>{toPlainString(balanceChange)}</span>,
					},
				],
			},
		],
